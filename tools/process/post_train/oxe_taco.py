...
from utils import relabel_bridge_actions, binarize_gripper_actions, rel2abs_gripper_actions

os.environ['CUDA_VISIBLE_DEVICES'] = '-1'

# Load the dataset
dataset_dirs = "/share/project/lxh/datasets/tf_datasets/taco_play/0.1.0"
...
base_output_dir = "/share/project/yuqi.wang/datasets/processed_data/oxembodiment/taco_play"
...
for episode in tqdm(ds_all_dict, desc="Processing episodes", unit="episode"):
    ...
    gripper_dir = os.path.join(episode_dir, 'grippers')
    ...
    for i, step in tqdm(enumerate(episode["steps"]), desc=f"Processing episode {name}", total=len(episode["steps"]), unit="step"):
        ...
        action = step["action"]["rel_actions_world"]

        gripper_action = tf.clip_by_value(action[-1:], 0, 1)
        ...
        image = observation["rgb_static"]
        image = Image.fromarray(image.numpy())
        gripper = observation["rgb_gripper"]
        gripper = Image.fromarray(gripper.numpy())

        
        # Save images to the episode folder
        image_filename = os.path.join(image_dir, f"{i:04d}.jpg")
        image.save(image_filename)
        gripper_filename = os.path.join(gripper_dir, f"{i:04d}_gripper.jpg")
        gripper.save(gripper_filename)
