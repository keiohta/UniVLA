...
# Load the dataset
dataset_dirs = "/share/project/lxh/datasets/tf_datasets/berkeley_autolab_ur5/0.1.0"
...
base_output_dir = "/share/project/yuqi.wang/datasets/processed_data/oxembodiment/berkeley_autolab_ur5"
...
for episode in tqdm(ds_all_dict, desc="Processing episodes", unit="episode"):
    ...
    gripper_dir = os.path.join(episode_dir, 'gripper')
    ...
    for i, step in tqdm(enumerate(episode["steps"]), desc=f"Processing episode {name}", total=len(episode["steps"]), unit="step"):
        ...
        gripper_action = action["gripper_closedness_action"].numpy()
        ...
        gripper = observation["hand_image"]
        ...
        gripper_filename = os.path.join(gripper_dir, f"{i:04d}.jpg")
