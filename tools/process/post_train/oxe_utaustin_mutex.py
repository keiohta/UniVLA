...
# Load the dataset
dataset_dirs = "/share/project/lxh/datasets/tf_datasets/utaustin_mutex/0.1.0"
...
base_output_dir = "/share/project/yuqi.wang/datasets/processed_data/oxembodiment/utaustin_mutex"
...
for episode in tqdm(ds_all_dict, desc="Processing episodes", unit="episode"):
    ...
    for i, step in tqdm(enumerate(episode["steps"]), desc=f"Processing episode {name}", total=len(episode["steps"]), unit="step"):
        ...
        step_action = tf.concat(
            (
                action[:6],
                invert_gripper_actions(tf.clip_by_value(action[-1:], 0, 1)),
            ),
            axis=0,
        )
