...
# Load the dataset
dataset_dirs = "/share/project/lxh/datasets/tf_datasets/toto/0.1.0"
...
base_output_dir = "/share/project/yuqi.wang/datasets/processed_data/oxembodiment/toto"
...
for episode in tqdm(ds_all_dict, desc="Processing episodes", unit="episode"):
    ...
    for i, step in tqdm(enumerate(episode["steps"]), desc=f"Processing episode {name}", total=len(episode["steps"]), unit="step"):
        ...
        gripper_action = tf.cast(action["open_gripper"], tf.float32)
